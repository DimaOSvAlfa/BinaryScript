        ...
        while (i < code.length()) {
            if (code[i] == '#') {
                ...
                if (next_hash != std::string::npos) {
                    ...
                    reportError("SYNTAX", "Unclosed comment '#' detected", code.substr(i, 10));
                    break;
                }
            } else {
                clean_code += code[i];
            ...
            if (!filePath.empty()) {
                ...
                if (filePath.rfind("http://", 0) == 0 || filePath.rfind("https://", 0) == 0) {
                    ...
                    if (!systemCmd.empty()) {
                        ...
                        reportError("SYSTEM", "Opening links is not supported on your OS");
                    }
                }
                else {
                    if (mode == "run") {
                        ...
                        if (scriptFile.is_open()) {
                            ...
                            reportError("FILE", "Failed to read or find script", filePath);
                        }
                    }
                    else if (mode == "read") {
                        std::ifstream scriptFile(filePath);
                        if (scriptFile.is_open()) {
                            ...
                            reportError("FILE", "Failed to read or find file", filePath);
                        }
                    }
                    else if (mode == "edit") {
                        std::ifstream scriptFile(filePath);
                        ...
                        std::cout << "Write the modified code:" << std::endl << "> " << std::flush;
                        ...
                        std::cout << "Write the code:" << std::endl << "> " << std::flush;
                        std::string newContent;
                        std::getline(std::cin, newContent);
                        ...
                        if (outFile.is_open()) {
                            outFile << newContent;
                            outFile.close();
                            std::cout << "File successfully created and written." << std::endl;
                        } else {
                            reportError("FILE", "Failed to create and write file", filePath);
                        }
                    }
                    else {
        ...
        // Присваивание переменной 0 ...
            ...
            if (eq != std::string::npos) {
                ...
                if (!name.empty() && name.front() == '!') {
                    ...
                    std::cout << "Enter value for " << var_name << ": " << std::flush;
...
int main() {
    ...
    std::cout << "BinaryScript Engine v" VERSION " [EN_ver]" << std::endl;
    std::string line;
