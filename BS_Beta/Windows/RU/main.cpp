...
private:
    ...
    void runSimpleCmd(std::string cmd) {
            ...
            if (!filePath.empty()) {
                ...
                if (filePath.rfind("http://", 0) == 0 || filePath.rfind("https://", 0) == 0) {
                    ...
                    if (!systemCmd.empty()) {
                        ...
                        reportError("SYSTEM", "Opening links is not supported on your operating system");
                    }
                }
                else {
                    if (mode == "run") {
                        ...
                        if (scriptFile.is_open()) {
                            ...
                            reportError("FILE", "Failed to read or find the script", filePath);
                        }
                    }
                    else if (mode == "read") {
                        std::ifstream scriptFile(filePath);
                        if (scriptFile.is_open()) {
                            ...
                            reportError("FILE", "Failed to read or find the file", filePath);
                        }
                    }
                    else if (mode == "edit") {
                        std::ifstream scriptFile(filePath);
                        ...
                        std::cout << "Please write the modified code:" << std::endl << "> " << std::flush;
                        ...
                        if (outFile.is_open()) {
                            outFile << newContent;
                            outFile.close();
                            std::cout << "The file has been successfully saved." << std::endl;
                        } else {
                            reportError("FILE", "Failed to save changes to file", filePath);
                        }
                    }
                    else if (mode == "write") {
                        std::cout << "Please write the code:" << std::endl << "> " << std::flush;
                        std::string newContent;
                        std::getline(std::cin, newContent);
                        ...
                        if (outFile.is_open()) {
                            outFile << newContent;
                            outFile.close();
                            std::cout << "The file has been successfully created and written." << std::endl;
                        } else {
                            reportError("FILE", "Failed to create and write to file", filePath);
                        }
                    }
                    else {
                        reportError("SYNTAX", "Unknown file operation mode (run, read, edit, write)", mode);
                    }
                }
            }
        }
        // Вывод 2([x]) - ТЕКСТ/СТРОКИ/СИМВОЛЫ
            ...
            if (isNumber) {
                reportError("TYPE ERROR", "2([x]) expects text but received a number", val);
            } else {
        ...
        // Вывод 2(x) - ЧИСЛА
            ...
            if (!isNumber) {
                reportError("TYPE ERROR", "2(x) expects a number but received text", val);
            } else {
                std::cout << val << std::endl << std::flush;
            }
        }
        // Присваивание переменной 0 ...
            ...
            if (eq != std::string::npos) {
                ...
                if (!name.empty() && name.front() == '!') {
                    ...
                    std::cout << "Please enter a value for " << var_name << ": " << std::flush;
        ...
        while (i < code.length()) {
            ...
            if (code[i] == '6' || code[i] == '8' || code[i] == '3' || code[i] == '4') {
                ...
                if (semi == std::string::npos) {
                    reportError("SYNTAX", "Missing ';' for block", code.substr(i, 10));
                    break;
                }
